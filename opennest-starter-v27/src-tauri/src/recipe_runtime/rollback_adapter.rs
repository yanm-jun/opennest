...
fn best_effort_stop(app: &AppHandle, recipe: &OpenNestRecipe, remove_volumes: bool) -> Result<(), String> {
    match recipe.runtime.as_str() {
        "native-cli" if recipe.id == "openclaw" => {
            let message = native_cli::stop_openclaw(app).unwrap_or_else(|error| format!("best-effort stop failed: {error}"));
...
fn cleanup_runtime_artifacts(app: &AppHandle, recipe: &OpenNestRecipe, remove_data: bool) -> Result<(), String> {
    ...
    match recipe.runtime.as_str() {
        "native-cli" if recipe.id == "openclaw" => {
            ...
            if remove_data {
                remove_dir_if_exists(app, &recipe.id, &dir, "full OpenClaw uninstall")?;
            } else {
                remove_dir_if_exists(app, &recipe.id, &dir.join("cli"), "OpenClaw CLI uninstall")?;
...
pub fn rollback_failed_install(app: &AppHandle, recipe: &OpenNestRecipe) -> Result<RecipeStatus, String> {
    ...
    match recipe.runtime.as_str() {
        "native-cli" if recipe.id == "openclaw" => {
            process_manager::clear_record(app, &recipe.id).ok();
            remove_dir_if_exists(app, &recipe.id, &dir.join("cli"), "rollback OpenClaw CLI partial install")?;
        }
        "docker-compose" => {
