...
pub fn resolve_ports(app: &AppHandle, recipe: &OpenNestRecipe, apply: bool) -> Result<PortResolutionResult, String> {
    ...
    if recipe.id == "openclaw" && (current_status.run_state == "running" || current_status.health_state.as_deref() == Some("healthy")) {
        mappings = recipe
            .ports
            .iter()
            .map(|port| RecipePortMapping {
                host: "127.0.0.1".to_string(),
                requested_port: *port,
                resolved_port: *port,
                changed: false,
        ...
        let result = PortResolutionResult {
            ...
            message: "OpenClaw Desktop is already running on its managed local port.".to_string(),
