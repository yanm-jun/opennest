...
use super::{command_timeout, docker_compose, logs, node_runtime, paths, port_resolver, status_store};
...
fn timed_command(command: Command, label: &str, timeout_ms: u64) -> Result<String, String> {
...
fn check_docker(app: &AppHandle, recipe: &OpenNestRecipe) -> Option<ResourcePreflightCheck> {
    let requires = recipe.runtime == "docker-compose" || recipe.runtime == "external-compose" || bool_requirement(recipe, "docker");
    if !requires {
        return None;
    }

    match docker_compose::check_docker(app, &recipe.id) {
        Ok(report) => Some(pass(
            "docker",
            "Docker Desktop / Compose",
            true,
            "Docker CLI, Docker Compose, and Docker daemon are ready.",
            Some(format!(
                "{} | {} | {}",
                report.docker_version,
                report.compose_version,
                report.daemon_info.lines().next().unwrap_or_default()
            )),
        )),
        Err(error) => Some(fail(
            "docker",
            ...
            true,
            error,
            Some("Start Docker Desktop, wait until it is running, then retry.".to_string()),
        )),
    }
}
...
pub fn run(app: &AppHandle, recipe: &OpenNestRecipe) -> Result<ResourcePreflightReport, String> {
    ...
    if let Some(check) = check_docker(app, recipe) { checks.push(check); }
