...
use super::status::{RecipeSecretInput, RecipeStatus, RuntimeActionResult};
use super::{
    docker_compose, docker_status, external_compose, healthcheck, http_readiness, install_plan, logs, native_cli, node_runtime, process_manager, port_resolver, preflight_gate, progress_events, recipe_loader, resource_preflight, rollback_adapter, secret_redaction_registry, status_store, token_store,
};
...
pub fn check_environment(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    let result = match recipe.runtime.as_str() {
        "native-cli" => {
            logs::append(app, &recipe.id, "environment", &format!("checking {} Node runtime environment", recipe.name))?;
...
pub fn install(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    let result = match recipe.runtime.as_str() {
        "native-cli" => match native_cli::install_app(app, &recipe.id) {
            Ok(_) => {
                ...
                RuntimeActionResult::ok(&recipe.id, &format!("{} installed.", recipe.name), Some(current_status(app, &recipe)))
            }
...
pub fn start(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    let result = match recipe.runtime.as_str() {
        "native-cli" => match native_cli::start_app(app, &recipe.id, port_resolver::effective_primary_port(app, &recipe).unwrap_or(18789)) {
            Ok(pid) => {
                ...
                RuntimeActionResult::ok(
                    &recipe.id,
                    format!("{} gateway started and healthcheck passed. pid={pid}", recipe.name),
                    Some(current_status(app, &recipe)),
                )
            }
            Err(error) => fail_with_persisted_status(app, &recipe.id, error),
...
pub fn stop(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    let result = match recipe.runtime.as_str() {
        "native-cli" => match native_cli::stop_app(app, &recipe.id) {
            Ok(message) => {
...
pub fn open_dashboard(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    let result = match recipe.runtime.as_str() {
        "native-cli" => match native_cli::app_dashboard(app, &recipe.id, port_resolver::effective_primary_port(app, &recipe).unwrap_or(18789)) {
            Ok(_) => RuntimeActionResult::ok(&recipe.id, "Dashboard opened.", Some(current_status(app, &recipe))),
            Err(error) => fail_with_persisted_status(app, &recipe.id, error),
        },
...
pub fn run_doctor(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    if recipe.runtime == "native-cli" && recipe.doctor.is_some() {
        return match native_cli::app_doctor(app, &recipe.id) {
            Ok(_) => Ok(RuntimeActionResult::ok(
                &recipe.id,
                &format!("{} doctor completed. Review logs for details.", recipe.name),
                Some(current_status(app, &recipe)),
            )),
            Err(error) => Ok(fail_with_persisted_status(app, &recipe.id, error)),
        };
...
pub fn run_onboarding(app: &AppHandle, app_id: &str) -> Result<RuntimeActionResult, String> {
    ...
    if recipe.runtime == "native-cli" && recipe.doctor.is_some() {
        return match native_cli::app_onboarding(app, &recipe.id) {
            Ok(_) => Ok(RuntimeActionResult::ok(
                &recipe.id,
                &format!("Official {} onboarding opened.", recipe.name),
                Some(current_status(app, &recipe)),
            )),
            Err(error) => Ok(fail_with_persisted_status(app, &recipe.id, error)),
        };
