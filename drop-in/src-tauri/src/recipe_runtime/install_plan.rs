...
fn substitute_known_paths(template: &str, app_dir: &str) -> String {
    ...
    value = value.replace("${binary}", &format!("<resolved {} binary>", app_id));
    value
}
...
fn warnings(recipe: &OpenNestRecipe) -> Vec<String> {
    ...
    if recipe.onboarding.is_some() {
        warnings.push(format!("{} may still require official onboarding if token-only startup is not enough for the current upstream version.", recipe.name).to_string());
    }
